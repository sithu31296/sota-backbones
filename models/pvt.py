...
class Block(nn.Module):
    def __init__(self, dim, head, reduction_ratio=1, mlp_ratio=4):
        super().__init__()
...
class PVTv2(nn.Module):
    def __init__(self, model_name: str = 'B1', pretrained: str = None, num_classes: int = 1000, *args, **kwargs) -> None:
        ...
        self.block1 = nn.ModuleList([
            Block(embed_dims[0], 1, 8, 8)
        for _ in range(depths[0])])
        self.norm1 = nn.LayerNorm(embed_dims[0])

        self.block2 = nn.ModuleList([
            Block(embed_dims[1], 2, 4, 8)
        for _ in range(depths[1])])
        self.norm2 = nn.LayerNorm(embed_dims[1])

        self.block3 = nn.ModuleList([
            Block(embed_dims[2], 5, 2, 4)
        for _ in range(depths[2])])
        self.norm3 = nn.LayerNorm(embed_dims[2])

        self.block4 = nn.ModuleList([
            Block(embed_dims[3], 8, 1, 4)
